...
export function MapScreen({ character, onCharacterUpdated, onStoryChapterChanged, initialAdminSection }: MapScreenProps) {
  ...
  async function maybeShowRouteCompletionSummary(options: { refreshFindings?: boolean } = {}) {
    ...
    shownRouteCompletionSummaryKeysRef.current.add(summary.key);
    setRouteCompletionSummary(summary);
  }
  ...
  useEffect(() => {
    setRouteCompletionSummary(null);
  }, [route.id]);
  ...
  }, [allMarkerRouteLinks, authoredToasts, completedEventIds, completedRouteId, currentRouteProgress?.id, currentRouteProgress?.source_marker_id, mapEvents, markers, progressPercent, route, routeDirection, routeFindings, selectedChapter, selectedSeason, storyFlags]);

  useEffect(() => {
    if (completedRouteId !== route.id || routeDirection === "reverse" || progressPercent < 100) {
      return;
    }

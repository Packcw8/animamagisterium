...
export async function saveWorldMapSetting(input: Partial<WorldMapSetting>) {
  ...
  const values = {
    ...
    width: Math.max(320, Number(input.width) || 4096),
    height: Math.max(320, Number(input.height) || 2763),
    is_active: input.is_active ?? true,
    created_by: input.id ? input.created_by ?? user?.id ?? null : user?.id ?? null,
    updated_at: new Date().toISOString(),
  };
...
export async function saveMiniMap(input: Partial<MiniMap>) {
  ...
  const values = {
    ...
    entry_toast_title: input.entry_toast_title?.trim() || null,
    entry_toast_message: input.entry_toast_message?.trim() || null,
    entry_sound_url: input.entry_sound_url?.trim() || null,
...
export async function createDialogueChoice(input: Omit<StoryDialogueChoice, "id" | "created_at" | "updated_at" | "unlock_marker_id" | "update_notification_title" | "update_notification_body" | "restore_health" | "restore_stamina" | "restore_mana" | "choice_group_key" | "choice_group_lock_message" | "hide_when_group_locked" | "set_story_flag_key" | "set_story_flag_value" | "repeatable" | "hide_after_selected" | "disable_after_selected" | "selected_message" | "travel_target_type" | "travel_target_marker_id" | "travel_target_mini_map_id" | "travel_target_spawn_marker_id" | "consume_required_item"> & Partial<Pick<StoryDialogueChoice, "unlock_marker_id" | "update_notification_title" | "update_notification_body" | "restore_health" | "restore_stamina" | "restore_mana" | "choice_group_key" | "choice_group_lock_message" | "hide_when_group_locked" | "set_story_flag_key" | "set_story_flag_value" | "repeatable" | "hide_after_selected" | "disable_after_selected" | "selected_message" | "travel_target_type" | "travel_target_marker_id" | "travel_target_mini_map_id" | "travel_target_spawn_marker_id" | "consume_required_item">>) {
  const values = {
    ...input,
    consume_gold: input.consume_gold ?? 0,
    consume_required_item: input.consume_required_item ?? false,

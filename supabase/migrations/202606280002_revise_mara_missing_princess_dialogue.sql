alter table public.story_dialogue_choices
  ...
  add column if not exists hide_when_group_locked boolean default false,
  add column if not exists set_story_flag_key text,
  add column if not exists set_story_flag_value boolean;
...
begin
  ...
  select m.id, m.npc_id, m.x_percent, m.y_percent, m.parent_marker_id
  into v_mara_marker_id, v_mara_npc_id, v_mara_x, v_mara_y, v_mara_parent_marker_id
  ...
  if v_missing_marker_id is null then
    ...
    values (
      ...
      'The town whispers about a royal disappearance. Commander Thorn may know where the search begins.',
      least(96, greatest(4, coalesce(v_mara_x, 50) + 4)),
      least(96, greatest(4, coalesce(v_mara_y, 50))),
      true,
      false,
      true,
      'story_locked',
      'Speak with Mara at Raven''s Perch.',
      v_mini_map_id,
      v_mara_parent_marker_id,
      'campaign_missing_princess',
      0,
      false,
      0,
      0,
    ...
    returning id into v_missing_marker_id;
  else
    update public.map_markers
    set
      type = 'Story',
      title = 'The Missing Princess',
      description = 'The town whispers about a royal disappearance. Commander Thorn may know where the search begins.',
      is_active = true,
      is_interactable = true,
      is_unlocked = false,
      lock_type = 'story_locked',
      lock_message = 'Speak with Mara at Raven''s Perch.',
      quest_key = 'campaign_missing_princess',
      story_order = 0,
      ...
      season_number = 1,
      chapter_number = 1,
      icon_label = coalesce(nullif(icon_label, ''), '!'),
      icon_color = coalesce(nullif(icon_color, ''), '#d9a441'),
      marker_size = coalesce(marker_size, 130),
      interaction_radius_percent = greatest(coalesce(interaction_radius_percent, 4), 30),
      updated_at = now()
    where id = v_missing_marker_id;
  end if;
...
But if it's coin you're after...

There's only one job anyone's talking about.$dialogue$,
...
The King has offered a generous reward for anyone who brings back information leading to her safe return.

Since then...

It feels like half the kingdom has passed through Raven's Rest looking for her.$dialogue$,
...
They're never far from a good argument.$dialogue$,
  ...
  insert into public.story_dialogue_nodes
    (event_id, marker_id, node_key, title, npc_name, npc_id, dialogue_text, is_start, is_ending, allow_end_chat, end_completes_event, sort_order)
  values
    (null, v_mara_marker_id, 'mara_ravens_perch_09_commander_thorn', 'Commander Thorn', 'Mara', v_mara_npc_id,
$dialogue$Commander Thorn has established a search headquarters just outside Hearthguard.
...
If you're serious about earning that reward...

That's where I'd start.$dialogue$,
  ...
  values
    ...
    (n1, 'Is there any way to make some gold?', 'go_to_node', n3, 2),
    (n2, 'What happened?', 'go_to_node', n4, 1),
    (n2, 'Is there any way to make some gold?', 'go_to_node', n3, 2),
    (n3, 'What job?', 'go_to_node', n5, 1),
    (n4, 'What job?', 'go_to_node', n5, 1),
  ...
  values
    ...
    (n3, 'Maybe another time.', 'end_conversation', 2),
  ...
  values (
    ...
    v_missing_marker_id,
    'Story Updated',
    'The Missing Princess has been marked on your map.',
    'missing_princess_campaign_started',
    true,
    'mara_missing_princess_lead',
    'Mara has already pointed you toward the missing princess lead.',
    false,
    true,
    true,
    'Mara has already pointed you toward the missing princess lead.',
  ...
  insert into public.story_dialogue_choices (node_id, button_text, action, sort_order)
  values
    (n9, 'Thank you.', 'end_conversation', 2);
end $$;

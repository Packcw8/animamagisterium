...
begin
  ...
  if v_choice.reward_item_id is not null then
    v_item_id := v_choice.reward_item_id;
    v_item_quantity := greatest(1, coalesce(v_choice.reward_item_quantity, 1));

    insert into public.player_inventory(user_id, character_id, item_id, quantity)
    values (v_user_id, p_character_id, v_item_id, v_item_quantity)
  ...
  loop
    ...
    v_item_quantity := greatest(1, coalesce(v_reward.quantity, 1));

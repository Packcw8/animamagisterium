-- Recover "The Silent Corpse" after accidental deletion.
-- This recreates the map_events row if missing, then rebuilds the dialogue tree and one-time satchel rewards.
-- After running, attach the event to the desired walking path/percent in Admin if the route lookup below does not match your intended trail.
do $$
declare
  v_event_id uuid;
  v_route_id uuid;
...
begin
  ...
  where lower(name) like '%iron%'
    and lower(name) like '%raven%'
  ...
  if v_event_id is null then
    ...
    values (
      ...
      v_route_id,
      35,
      'A silent traveler lies beside the road.',
      '[]'::jsonb,
      0,
      0,
      null,
    ...
    update public.map_events
      set
        event_type = 'dialogue',
        dialogue_text = coalesce(dialogue_text, 'A silent traveler lies beside the road.'),
        choices = '[]'::jsonb,
        reward_xp = 0,
        reward_gold = 0,
        reward_item = null,
  ...
  if v_common_sword_id is null then
    raise exception 'Common Sword item was not found. Create the item first, then rerun this recovery.';
  end if;
  ...
  if v_healing_potion_id is null then
    raise exception 'Healing Potion / Health Potion item was not found. Create the item first, then rerun this recovery.';
  end if;
